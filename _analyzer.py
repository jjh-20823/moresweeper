...
class Board(object):
    ...
    def __init__(self, board: list):
        ...
        self.result['bv'] = self.result[
            'op'] + self.result['row'] * self.result['column'] - sum(
                sum(self.marker, []))
        self.result['is'] = self.get_openings_or_islands(
            self.is_not_opening_or_mine)
    ...
    def get_difficulty(self) -> str:
        ...
        size = (self.result['row'], self.result['column'],
                self.result['mines'])
        difficulty_ref = {
            (8, 8, 10): 'beg',
            (16, 16, 40): 'int',
    ...
    def filtered_adjacent(self, row: int, col: int, filters: bool):
        ...
        adjacent = [(row - 1, col - 1), (row, col - 1), (row + 1, col - 1),
                    (row - 1, col), (row + 1, col), (row - 1, col + 1),
                    (row, col + 1), (row + 1, col + 1)]
        for r, c in adjacent:
            if 0 <= r < self.result['row'] and 0 <= c < self.result[
                    'column'] and filters(r, c):
                yield r, c
    ...
    def recur_mark(self, row: int, col: int, condition: bool):
        ...
        if condition(row, col):
            for next_row, next_col in self.filtered_adjacent(
                    row, col, self.is_not_marked):
...
class Record(Board):
    ...
    def __init__(self, board: list, action: list, initial: list = None):
        ...
        self.marker = [[0 for _ in range(self.result['column'])]
                       for _ in range(self.result['row'])]
        ...
        self.result['flags'], self.result['unflags'], self.result[
            'misflags'], self.result['misunflags'] = 0, 0, 0, 0
        self.result['ce'], self.result['solved_bv'], self.result[
            'solved_op'] = 0, 0, 0
    ...
    def __find_final(self, start: int, direction: int, row: int,
                     col: int) -> int:
        ...
        while 0 <= final < len(self.action) and (
                self.action[final][3] -
                self.action[start][3]) * direction <= self._threshold and not (
                    self.action[final][0] == 1 and self.action[final][1] == row
                    and self.action[final][2] == col):
            final += direction
        return final, 0 <= final < len(
            self.action) and (self.action[final][3] - self.action[start][3]
    ...
    def __refine_action(self, current):
        ...
        final, found = self.__find_final(release, +1, tag_row,
                                         tag_col)  # move forwards
        if found:
            self.action[final][0] = 4
        else:
            final, found = self.__find_final(release, -1, tag_row,
                                             tag_col)  # move backwards
            if found:
                self.action[final][0] = 4
    ...
    def __is_opening_fully_opened(self, row: int, col: int) -> bool:
        ...
        state = True
        for next_row, next_col in self.filtered_adjacent(
                row, col, self.is_opening):
            if self.op_marker[next_row][next_col] == 0:
                self.op_marker[next_row][
                    next_col] == 1  # mark the opening (excluding edges)
                state = state and self.__is_opening_fully_opened(
                    next_row, next_col)  # truncate if state is False already
    ...
    def __deal_with_click(self, row: int, col: int) -> bool:
        ...
        if self.board[row][col] == '9':  # step on a mine, oops
            ...
            self.recur_mark(
                row, col,
                self.is_opening)  # mark everything that is the opening
            ...
            self.result['solved_bv'] += ('0' not in [
                self.board[r][c]
                for r, c in self.filtered_adjacent(row, col, self.is_opening)
            ])
            self.marker[row][col] = 1
    ...
    def __deal_with_flag(self, row: int, col: int):
        ...
        if self.marker[row][col] == 0:  # flagging
            ...
            self.result[
                'misflags'] += self.board[row][col] != '9'  # count misflags
            ...
            self.result['unflags'] += (
                not misunflag_tag)  # count unflags (opposite to misunflags)

        return True  # any direct click is effective

    def __deal_with_chord(self, row: int, col: int):
        """Deal with chording operation (corresponding to opcode 4)."""
        adjacent_flagged = list(
            self.filtered_adjacent(row, col, self.is_marked_flag))
        adjacent_unopened = list(
            self.filtered_adjacent(row, col, self.is_not_marked))
        if len(adjacent_flagged) != int(self.board[row][col]) or len(
                adjacent_flagged) == 0 or len(adjacent_unopened) == 0:
    ...
    def get_action_detail(self):
        ...
        self.result['path'], self.result['left'], self.result[
            'right'], self.result['double'], current, last = 0, 0, 0, 0, 0, 0
        while current < len(self.action):
            if self.action[current][0] in [0, 1, 4]:
                self.result['path'] += math.sqrt(
                    (self.action[current][1] - self.action[last][1])**2 +
                    (self.action[current][2] - self.action[last][2])**
                    2)  # Euclidean path
                ...
                last = current
            current += 1
        self.result['cl'] = self.result['left'] + self.result[
            'right'] + self.result['double']
    ...
    def prepare_initial_board(self, initial: list):
        ...
        self.marker = [[int(v) for v in each_row]
                       for each_row in initial]  # reset the marker by raw data
    ...
    def get_record_detail(self):
        ...
        self.result['bvs'] = _divide(self.result['solved_bv'],
                                     self.result['rtime'])
        self.result['cls'] = _divide(self.result['cl'], self.result['rtime'])
        self.result['est'] = _divide(
            self.result['rtime'], self.result['solved_bv']) * self.result['bv']
        self.result['rqp'] = _divide(self.result['rtime'] + 1,
                                     self.result['bvs'])
        self.result['qg'] = _divide(self.result['rtime']**1.7,
                                    self.result['solved_bv'])
        self.result['iome'] = _divide(self.result['solved_bv'],
                                      self.result['path'])
        self.result['ces'] = _divide(self.result['ce'], self.result['rtime'])
        self.result['corr'] = _divide(
            self.result['ce'] - self.result['misflags'] -
            self.result['unflags'] - self.result['misunflags'] -
            (self.result['bv'] != self.result['solved_bv']), self.result['cl'])
        self.result['thrp'] = _divide(self.result['solved_bv'],
                                      self.result['ce'])
        self.result['ioe'] = _divide(self.result['solved_bv'],
                                     self.result['cl'])

        mode_ref = {'beg': 1, 'int': 2, 'exp-v': 3, 'exp-h': 3}
        if self.result['difficulty'] in mode_ref:
            mode = mode_ref[self.result['difficulty']]
            self.result['stnb'] = _divide(
                87.420 * (mode**2) - 155.829 * mode + 115.708,
                self.result['qg'] *
                math.sqrt(self.result['solved_bv'] / self.result['bv']))
